...
import { Button } from "@/components/ui/button";
import { Calendar, ArrowRight } from "lucide-react";
import { publicBlogApi } from "@/api/publicBlog.api";
...
const Blog = () => {
  ...
  const loadBlogs = async () => {
    setLoading(true);
    const res = await publicBlogApi.getBlogs({
  ...
  const loadCategories = async () => {
    const res = await publicBlogApi.getCategories();
  ...
  return (
    <div className="min-h-screen">
      {/* Hero */}
      <section className="bg-gradient-to-br from-primary/10 to-secondary/10 section-padding text-center">
        <h1 className="heading-xl mb-4">Health Blog & Resources</h1>
        <p className="text-xl max-w-3xl mx-auto">
          Expert insights on digestive health, liver care, and wellness
        </p>
      </section>
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {blogs.map(blog => (
                <Card
                  key={blog._id}
                  className="card-medical hover:scale-105 transition-transform h-full flex flex-col"
                >
                  <CardHeader>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
                      <Calendar className="w-4 h-4" />
                    ...
                    <div className="inline-block px-3 py-1 bg-primary/10 text-primary rounded-full text-xs mb-3 w-fit">
                      {blog.category}
                    </div>

                    <CardTitle className="text-xl">
                      {blog.titleHindi || blog.titleEnglish}
                    </CardTitle>
